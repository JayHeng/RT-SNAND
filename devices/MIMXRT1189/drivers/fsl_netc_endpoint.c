...
static void EP_GetBaseResource(ep_handle_t *handle, netc_hw_si_idx_t si)
{
    ...
    handle->hw.portGroup.eth  = (NETC_ETH_LINK_Type *)((uintptr_t)handle->hw.portGroup.port + 0x1000U);
...
#endif
{
    ...
    if (enable)
    {
        ...
        if ((TMR0_BASE->TMR_CTRL & ENETC_PF_TMR_TMR_CTRL_TE_MASK) != 0U)
        {
...
status_t EP_TxTGSConfigAdminGcl(ep_handle_t *handle, netc_tb_tgs_gcl_t *config)
{
    ...
    {
        netc_cmd_bd_t cmdBd = {0};
        uint32_t timeLow, timeHigh, cycleTime;
        uint64_t minBaseTime;
        ...
        minBaseTime = ((uint64_t)timeHigh << 32U) + timeLow + 100100U + (2U * cycleTime);
        ...
        {
            config->baseTime +=
                (((uint64_t)ceil((double)(minBaseTime - config->baseTime) / (double)config->cycleTime))) *
                config->cycleTime;
        }
    }
#endif
