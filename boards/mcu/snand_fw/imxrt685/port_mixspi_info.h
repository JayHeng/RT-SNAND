        ...
        case CLKCTL0_MAINCLKSELB_SEL(0):
            switch (mainClkSelA)
            {
                case CLKCTL0_MAINCLKSELA_SEL(0):
                    snand_printf("SNAND: CPU Clk Source from MAINCLKSELA 2'b00 - FFRO clock (48/60m_irc) div4 Clock %dHz.\r\n", CLOCK_GetFFroFreq()/4);
                    break;
                case CLKCTL0_MAINCLKSELA_SEL(1):
                    snand_printf("SNAND: CPU Clk Source from MAINCLKSELA 2'b01 - OSC clock %dHz.\r\n", CLOCK_GetXtalInClkFreq());
                    break;
